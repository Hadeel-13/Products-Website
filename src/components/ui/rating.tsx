...
const ratingVariants = cva("flex gap-x-1.5", {
  variants: {
    size: {
      xs: "[&_label>svg]:size-4",
    ...
    color: {
      ...
      error:
        "[&_.selected-star>svg]:text-error [&_.selected-star>svg]:fill-error",
    },
  },
  defaultVariants: {
    size: "md",
...
const Rating = ({
  ...
  selectOnHover = false,
  ...props
}: React.ComponentProps<"div"> & RatingProps) => {
  ...
  return (
    <div
      className={cn(
        ratingVariants({ size, color }),
        "aria-disabled:pointer-events-none aria-disabled:opacity-90",
        className
      )}
      {...props}
      role="radiogroup"
